        ...
        <div id="display_item row justify-content-center">
            ...
            <div class="d-grid gap-2 d-md-flex  justify-content-md-start" align="center">
                ...
                <tbody>
                    ...
                    $od_detail = "SELECT design.design_id, design.design_ref_pic, design.design_position, design.design_color, design.design_size, design.design_detail, design.order_id, book_detail.date, book_detail.time, book_detail.position, book_detail.order_size, book_detail.order_detail, book_detail.start_time, book_detail.end_time, book_detail.status, book_detail.artist_id, book_detail.user_id, book_detail.design_pic, payment.payment_amount, payment.payment_status FROM `design` LEFT JOIN book_detail ON design.order_id = book_detail.order_id LEFT JOIN `payment` ON design.order_id = payment.order_id  WHERE design.order_id = $orderNumber"; //่join ตาราง orderdetalis,orders,tbl_product
            ...
            <div id="hero2" class="row flex-lg-row-reverse justify-content-center align-items-center g-5 py-5">
                ...
                <?php //echo $rs[2]; ?>
                    ...
                    <p class="lead">ตำแหน่งที่สัก: <?php echo $rs[2]; ?></p>
                    <p class="lead">ขนาดของลายสัก: <?php echo $rs[4]; ?> cm</p>
                    <p class="lead">รายละเอียดเพิ่มเติม: <?php echo $rs[5]; ?></p>
                    <p class="lead">ค่ามัดจำ: <?php echo $rs[18]; ?> บาท (<?php echo $rs[19]; ?>)</p>
                    <form action="pay2.php" method="post">
                    <input type="hidden" name="price" value="<?php echo $rs[18]; ?>">
                    <input type="hidden" name="idxx" value="<?php echo $rs['order_id']; ?>">
                    <?php 
                ...
                <div class="col-10 col-sm-8 col-lg-6" style="margin-bottom: 0;">
                <h4>รูปที่ส่งมาให้ร้าน</h4>
                    <img src="img/<?php echo $rs[1]; ?>" class="d-block mx-lg-auto img-fluid" alt="Bootstrap Themes" width="400" height="400" loading="lazy">
               <br> <h4>รูปที่ร้านออกแบบ</h4> <img src="img/<?php echo $rs['design_pic']; ?>" class="d-block mx-lg-auto img-fluid" alt="Bootstrap Themes" width="400" height="400" loading="lazy">
