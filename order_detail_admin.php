        ...
        <div id="display_item row justify-content-center">
            <table class="table">
                <div class="d-grid gap-2 d-md-flex  justify-content-md-start" align="center">
                    <a href="admin_book.php" style="text-decoration:none;">
                ...
                <tbody>
                    ...
                    $od_detail = "SELECT book_detail.order_id, book_detail.date, book_detail.time, book_detail.position, book_detail.order_size, book_detail.start_time, book_detail.end_time, book_detail.order_detail, book_detail.time_stamp, tatto.tatto_pic, payment.payment_amount, payment.payment_status, artist.artist_id, artist.artist_nickname FROM book_detail LEFT JOIN upload_picture ON book_detail.order_id = upload_picture.order_id LEFT JOIN design ON book_detail.order_id = design.order_id LEFT JOIN payment ON book_detail.order_id = payment.order_id LEFT JOIN tatto ON (book_detail.tatto_id = tatto.tatto_id) LEFT JOIN artist ON book_detail.artist_id = artist.artist_id  where book_detail.order_id = $orderNumber"; //่join ตาราง orderdetalis,orders,tbl_product
                                ...
                                <div id="hero2" class="row flex-lg-row-reverse justify-content-center align-items-center g-5 py-5">
                                    ...
                                    <div class="col-lg-6 justify-content-center align-items-center">
                                        <form action="edit_order.php" method="POST">
                                            ...
                                            <p class="lead">เวลาที่จอง: <input type="text" name="time" style="border-radius: 10px; padding:5px; border:solid 1px black; width: 35%;"  value="<?php echo $rs[2]; ?>"></p>
                                            <p class="lead">ตำแหน่งที่สัก: <input type="text" name="position" style="border-radius: 10px; padding:5px; border:solid 1px black; width: 35%;"  value="<?php echo $rs[3]; ?>"></p>
                                            <p class="lead">ไซส์ของลายสัก: <?php echo $rs[4]; ?>
                                            <p class="lead">รายละเอียดเพิ่มเติม: <?php echo $rs[7]; ?></p>
                                            <input type="hidden" name="id" style="border-radius: 10px; padding:5px; border:solid 1px black;"  value="<?php echo $rs[0]; ?>">
                                           
                                            <p class="lead">ค่ามัดจำ: <?php echo $rs[10]; ?> บาท (<?php echo $rs[11]; ?>)</p> 

                                            <p class="lead">ช่าง: <?php echo $rs[13]; ?></p>
                                            ...
                                            <button class="w-4 btn btn-lg btn-danger" style="width:120px; font-size:15px; border-radius: 25px; margin-left: 25%; margin-top:0;" type="submit" name="edit_book" autocomplete="on" onclick="location.href='pay.php'">บันทึก</button>
                                    </form>   
                                    </div>
                                    <div class="col-10 col-sm-8 col-lg-6">
                                        <img src="img/<?php echo $rs[9]; ?>" class="d-block mx-lg-auto img-fluid" alt="Bootstrap Themes" width="500" height="500" loading="lazy">
                                    </div>
                                    
                                    <button class="w-4 btn btn-lg btn-primary" style="width: 200px; font-size:15px; border-radius: 25px; margin-left: 10px; margin-top: 0;" type="submit" name="edit_book4" onclick="location.href='edit_after_order.php?oid=<?php echo $rs['order_id']; ?>'" autocomplete="on" >บันทึกชำระเงินหลังสัก</button>
