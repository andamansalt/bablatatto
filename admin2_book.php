    ...
    <div class="container-fluid">
      ...
      <a href=" index_user.php" class="d-flex align-items-center col-md-3 mb-2 mb-md-0 text-dark text-decoration-none">
        ...
        <div class="box-se d-flex col-8 justify-content-start my-3">
            <h5 class="text-1" style="margin-right: 10px;">สถานะ</h5>
           <!--  <select id="dropdown-menu" name="dropdown-menu">
            ...
            <input type="button" class="detail-button" name="fillter" style="margin: 0;" value="ค้นหา" onclick="">
            <div class="col-10 d-flex justify-content-start">
                    
                </div> -->
                <button type="button" class="btn btn-outline-danger" style="margin-right: 10px;" onclick="location.href='admin_book.php'">ทั้งหมด</button>
                    <button type="button" class="btn btn-danger" style="margin-right: 10px;" onclick="location.href='admin2_book.php'">รอดำเนินการ</button>
                    <button type="button" class="btn btn-outline-danger" style="margin-right: 10px;" onclick="location.href='admin3_book.php'">จองสำเร็จ</button>
        ...
        <div class="container">
            <div class="row">
                <div class="col-12">
                    ...
                    $order = mysqli_query($conn, "SELECT book_detail.*, tatto.*, payment.*, artist.*, user.* FROM book_detail LEFT JOIN upload_picture ON book_detail.order_id = upload_picture.order_id LEFT JOIN design ON book_detail.order_id = design.order_id LEFT JOIN payment ON book_detail.order_id = payment.order_id LEFT JOIN tatto ON (book_detail.tatto_id = tatto.tatto_id) LEFT JOIN artist ON book_detail.artist_id = artist.artist_id LEFT JOIN user ON book_detail.user_id = user.user_id WHERE book_detail.book_type = 1 AND book_detail.status = 'รอดำเนินการ' ORDER BY book_detail.date, book_detail.time"); //เลือกข้อมูลที่ต้องการจากใน orders
                    ...
                    ?>
                            <!-- <tr>
                                <td></td>
                                <td></td>
                                <td><a class="btn btn-success" href="order_detail.php?oid=<?php echo $rs['order_id']; ?>">รายละเอียด</a></td>
                            </tr> -->
                            <div class="content-1 d-flex justify-content-start">
                                <p class="c name"><?php echo $rs['order_id']; ?></p>
                                ...
                                <input type="button" class="detail-button" value="รายละเอียด" onclick="location.href='order_detail_admin.php?oid=<?php echo $rs['order_id']; ?>'">
                            </div>
                    <?php }
                    }
                    ?>
